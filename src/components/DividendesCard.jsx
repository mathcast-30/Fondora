...
function DividendesCard({ dividendes, totalDouzeMois, syntheseDouzeMois, valorisationTotale, positions, onAjouter, onSupprimer }) {
    ...
    const handleSupprimer = (dividende) => {
        if (window.confirm(`Supprimer ce dividende de ${formatMontant(dividende.montant)} du ${new Date(dividende.date).toLocaleDateString('fr-FR')} ?`)) {
            onSupprimer(dividende.id)
        }
    }
    ...
    return (
        <div className="space-y-4 mb-6">
            {/* Estimation automatique basée sur les positions détenues */}
            <EstimationDividendesPanel positions={positions} />

            {/* Graphiques */}
            <DividendesGraphiques dividendes={dividendes} synthese={synth} />

            {/* Historique + fiscalité réelle */}
            <div className="bg-white rounded-xl p-5 shadow-sm">
                <div className="flex items-center justify-between mb-4">
                    <h3 className="text-navy font-semibold">Dividendes perçus (réel)</h3>
                        ...
                        {dividendes.map((d) => (
                            <div key={d.id} className="flex items-center justify-between text-sm py-1.5">
                                ...
                                <div className="flex items-center gap-2">
                                    ...
                                    <button onClick={() => handleSupprimer(d)} className="text-gray-300 hover:text-red-500">
                                        <Trash2 size={14} />
                                    </button>
                                </div>
                            </div>
